// ProjectStore.swift
//
// Persists Projects (per-song pad workspaces, see
// ToneForgeEngine/Projects/ProjectSnapshot.swift) as JSON under
// `Documents/projects/` — the SessionStore pattern: one file per
// project named by id, atomic writes, prettyPrinted + sortedKeys so
// files diff cleanly, list() skips corrupt files silently.
//
// Two kinds of file live here:
//   * durable projects  — `{id}.json`, listed in Library → Projects.
...
//     out of `list()` so the Library shows only named saves.
...
public final class ProjectStore {
    ...
    /// `{Documents}/projects/`. Created on first access.
    ...
    /// `{Documents}/projects/working/`. Created on first access.
