...
// vocoder output, baked transforms). Layout:
//
//   {Documents}/samples/{uuid}.wav    48 kHz mono Float32 payload
//   {Documents}/samples/{uuid}.json   PadSampleMetadata sidecar
//
// Documents (not Caches) because these are user creations — the OS
// must never evict them. They are still device-local: the metadata's
// `neverUpload` tripwire plus ComplianceTests keep every upload path
// away from this directory.
//
// @MainActor because it publishes the sample list to SwiftUI; the
// actual WAV encode/decode runs off-main (`save` is async) so an 8 s
// write never blocks the grid. Root + FileManager are injectable —
// tests run against a temp directory (LayerStore pattern).
...
public final class PadSampleStore: ObservableObject {
    ...
    /// `{Documents}/samples/`. Created on first access.
    ...
    /// `sampleRate`, `channels` from the payload (single source of
    /// truth), enforces the 8 s compliance cap, writes the WAV
    /// off-main, then the sidecar, then publishes.
    @discardableResult
    public func save(
    ...
    /// its stored rate (SampleScheduler converts to the canonical
    /// format at ingest, its single resample point).
    ...
    /// Total bytes on disk across all payloads + sidecars (storage
    /// browser, P7).
    ...
    /// Re-scan the sidecars. Bad JSON is skipped, not surfaced — one
    /// corrupt sidecar must not hide every other sample. A sidecar
    /// whose WAV is missing is treated as corrupt and skipped too.
    public func reload() {
        guard let dir = try? samplesDir(),
              let contents = try? fileManager.contentsOfDirectory(
                  at: dir,
