...
//
// Beat Capture (D-024): device-local outbound queue of user corrections
// to the drum classifier. Each record pairs the onset features with the
// model's original guess and the user's correction — training data for
// the server-side Core ML classifier.
//
// Posture: analysis features only, no audio. Corrections upload to the
// backend (opt-in) and clear locally on success; the large training
// corpus lives server-side, never on device.
...
public final class BeatTrainingStore: ObservableObject {
    ...
    /// Corrections queued for upload (newest last). Outbound queue, not a
    /// growing corpus — `flush` clears it on a successful upload and
    /// `maxBuffered` bounds it so a device never accumulates the large
    /// training set (that lives server-side).
