...
//
// CoreMIDI OUTPUT, ported from the mobile app (PERFORM_PARITY spec
// 2B): publishes a virtual MIDI source named "Tone Forge Jam" that
// external synths, drum machines, DAWs, and other apps subscribe to.
// Emits 24 PPQN timing clock + start/stop/continue so hardware follows
// the Jam transport.
//
...
// when clock-out is enabled) so headless sessions never open a
...
// thread-safe, but SessionController's clock driver calls in on main.
//
// NOTE (v1): clock pulses are driven by a main-thread timer, so tempo
// carries the timer's jitter — same v1 trade-off as mobile.
...
public final class MIDIOutTransport {
    ...
    public init(sourceName: String = "Tone Forge Jam") {
        ...
        // endpoint to peers that remember connections by ID. "ToJ2":
        // distinct from mobile's "ToJ1" so an iPad build running on
        // Apple-silicon macOS can't collide with the desktop app.
        MIDIObjectSetIntegerProperty(virtualSource, kMIDIPropertyUniqueID, 0x546F_4A32)
        isAvailable = true
    }
    ...
    /// SessionController driver computes `count` from MIDIClockGenerator.
