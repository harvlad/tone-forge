...
## D-035: Web-parity 64-pad kit flood + phantom pack-pad fix (empty workspace on no-snapshot songs)
...
- **Phantom purple "speaker" pads, root-caused.** Two purple tiles among
  ...
  every grid, live and triggerable. The same leak was independently
  root-caused on main in the same day's hardware session (D-034,
  `ProjectStateBridge.activateFresh`) — one mechanism, kept once. What
  this pass adds on top: running pad sequences are stopped BEFORE the
  swap (a pattern still driving a slot about to vanish would sound on
  with no pad able to stop it — the orphaned-voice class of bug), and
  the clear is pinned to PERSIST across relaunch (the phantoms lived in
  UserDefaults; an in-memory clear alone would resurrect them).
  Per-song slots still round-trip via working projects (D-030 auto-save);
...
`Sources/JamDesktopCore/Projects/ProjectStateBridge.swift`
(`activateFresh`, landed in D-034),
`Sources/JamDesktop/SessionController.swift`
(`loadAutoKit` pads/yieldToBorrow, `reloadKitForPadCountChange`),
`Sources/JamDesktop/Projects/ProjectCoordinator.swift`
(`songDidActivate` no-snapshot swap + sequence stop),
